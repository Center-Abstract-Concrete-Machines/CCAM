---
title: "[We Don't Know Yet] What a Cinema Can Do III"
subtitle: expanded cinema
pubDate: 2024-01-01
endDate: 2026-04-10
presentationalDates: April 10, 2026
time: 'Doors @ 7pm / Performances @ 8pm'
assProjectId: wdky
location: Public Works, 2141 W North Ave
image:
    url: ./images/wdky2.png
    alt: computationally generated intricate reticulation texture
type: Event
featured: true
draft: true
tags:
    - Onion City Film Festival
    - Public Works gallery
    - expanded cinema
    - experimental
    - intermedia
people:
    - label: performers
      list:
          - alterotics
          - aj-mcclennon
          - james-connolly  
    - label: special guests
      list: 
          - aria-pedraza    
          - midwest-rave-culture-archive    
    - label: CCAM production
      list:
          - kim-nucci
          - aj-mendelsohn
          - muindi-fanuel-muindi
          - garrett-laroy-johnson
    - label: Onion City Film Festival team
      list:
          - elise-schierbeek
          - nicky-ni
    - label: partners
      list:
          - onion-city
          - public-works
          - chicago-filmmakers
    - label: sponsors
      list:
          - sketchbook-brewing
          - marz-brewing
---

Onion City Film Festival and the Center for Concrete and Abstract Machines (CCAM), in collaboration with Public Works Gallery co-present a live performance program adjacent to the festival exploring “expanded cinema” in an attempt to put cinematic art to the tasks of resistance, rupture, and reconfiguration of mediatic experience.

We ask: how can experimental modes of animating sound and moving image reconfigure the possibility of our relating to one another anew?

CCAM and OCFF have curated three performers 

# Tickets

<LinkButton
    text="Tickets Available Here"
    url="https://chicagofilmmakers.org/checkout?cartToken=b-8qfGXctzR5qCl5OgUHfcwwpNQbWHwdSi_aYUzc"
/>

Tickets are $35 in advance and $40 at the door.

Tickets must be bought through the Onion City website. Because of zoning restrictions, guests without tickets at the door will be referred to buy tickets through the website.
